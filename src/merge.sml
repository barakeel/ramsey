...
(* -------------------------------------------------------------------------
   There is an ugly load command
   ------------------------------------------------------------------------- *)

(*
load "glue"; open aiLib kernel graph enum gen glue syntax sat;
load (selfdir ^ "/def/ramseyDefTheory");
load (selfdir ^ "/enumf/ramseyEnumTheory");
...
val a = snd (hd g358il');
val b = snd (hd g4416il');

val thm = IMPOSSIBLE_35 8 g4416il' R4416p (hd g358il');
val finalthm = PROVE_HYPL 
 (map (IMPOSSIBLE_35 8  g4416il' R4416p) g358il') R358p;
...
val _ = save_thm ("r45_degree8", finalthm2);

export_theory ();

load "r45_degree8Theory";
