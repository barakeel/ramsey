...
fun thyid_compare (thya,thyb) = 
  let 
    val (namea,stampa1,stampa2) = dest_thyid thya 
    val (nameb,stampb1,stampb2) = dest_thyid thyb
  in
    case String.compare (namea,nameb) of
      LESS => LESS
    | GREATER => GREATER
    | EQUAL => 
      (case Arbnum.compare (stampa1,stampb1) of
         LESS => LESS
       | GREATER => GREATER
       | EQUAL => Arbnum.compare (stampa2,stampb2))
  end          
    
structure Graph = struct type graph = 
  (thyid * thyid list) list * (thyid, thyid list) Redblackmap.dict
...
in
   ...
   fun fringe () =
     let val all_parents = List.map #2 (#1 (!theGraph))
         fun is_parent y = Lib.exists (Lib.op_mem thyid_eq y) all_parents
     in List.filter (not o is_parent) (List.map #1 (#1 (!theGraph)))
     end;
   fun first P = Lib.first P (#1 (!theGraph))
end
end; (* structure Graph *)
